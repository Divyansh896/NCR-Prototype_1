...
//Add a suplier
const supplierstorage = JSON.parse(localStorage.getItem('suppliers'));
...
function showSupplierPopup() {
    ...
    setTimeout(() => {
        supplierModal.querySelector('.modal-content-supplier').style.opacity = "1";
        supplierModal.querySelector('.modal-content-supplier').style.transform = "translate(-50%, -50%)";
    }, 10);
}

// Function to close supplier modal
function closeSupplierPopup() {
    const modalContent = supplierModal.querySelector('.modal-content-supplier');
    modalContent.style.opacity = "0"; // Fade-out effect
    modalContent.style.transform = "translate(-50%, -60%)"; // Adjust position for effect
    setTimeout(() => {
...
function SaveNewSup(){
    ...
    saveDataToLocalStorage(newsup);

    localStorage.setItem("newsup", JSON.stringify(suppliers));
}
...
addSupplierButton.addEventListener("click", function () {
    ...
    const newSupplierName = document.getElementById("supplierName").value;
    if (newSupplierName) {
