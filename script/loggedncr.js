...
function sendNotification(ncrNum) {
    const notification = { text: `NCR No. ${ncrNum} has been sent to the Purchasing department via Gmail for review and action.`, ischecked: false }
    ...
    let PRNotification = JSON.parse(localStorage.getItem("PRNotification"))

    // Add the new notification message
    ERNotification.push(notification);
    ADNotification.push(notification);
    PRNotification.push(notification);
    ...
    localStorage.setItem('PRNotification', JSON.stringify(PRNotification));
